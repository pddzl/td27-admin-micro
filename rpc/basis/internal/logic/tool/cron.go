...
func (cl *CronLogic) mapCronToResp(cron *toolModel.CronModel) *cron_pb.CronResp {
	if cron == nil {
		return nil
	}
...
func (cl *CronLogic) GetCron(in *common_pb.IdReq) (*cron_pb.CronResp, error) {
	...
	if err != nil { return nil, status.Errorf(codes.Internal, "get cron failed: %v", err) }
	if cron == nil { return nil, status.Errorf(codes.NotFound, "cron not found") }
...
func (cl *CronLogic) ListCron(in *common_pb.PageReq) (*cron_pb.ListCronResp, error) {
	...
	if err != nil { return nil, status.Errorf(codes.Internal, "list cron failed: %v", err) }
	resp := &cron_pb.ListCronResp{List: make([]*cron_pb.CronResp, 0, len(crons)), Total: count}
	for _, c := range crons { resp.List = append(resp.List, cl.mapCronToResp(c)) }
	return resp, nil
}

func (cl *CronLogic) CreateCron(in *cron_pb.CreateCronReq) (*common_pb.SuccessResp, error) {
	cron := &toolModel.CronModel{Name: in.Name, Expression: in.Expression}
	cron.Method = in.Method.String()
	if in.Strategy != nil { cron.Strategy = *in.Strategy }
	if in.Open != nil { cron.Open = *in.Open }
	if in.Comment != nil { cron.Comment = *in.Comment }
	if in.ExtraParams != nil {
		ep := toolModel.ExtraParams{Command: in.ExtraParams.Command}
		for _, t := range in.ExtraParams.TableInfo {
			ep.TableInfo = append(ep.TableInfo, toolModel.ClearTable{
				TableName: t.TableName, CompareField: t.CompareField, Interval: t.Interval,
			})
		}
		cron.ExtraParams = ep
...
func (cl *CronLogic) UpdateCron(in *cron_pb.UpdateCronReq) (*cron_pb.CronResp, error) {
	...
	if err != nil { return nil, status.Errorf(codes.Internal, "get cron failed: %v", err) }
	if cron == nil { return nil, status.Errorf(codes.NotFound, "cron not found") }
	if in.Name != nil { cron.Name = *in.Name }
	if in.Method != nil { cron.Method = in.Method.String() }
	if in.Expression != nil { cron.Expression = *in.Expression }
	if in.Strategy != nil { cron.Strategy = *in.Strategy }
	if in.Open != nil { cron.Open = *in.Open }
	if in.Comment != nil { cron.Comment = *in.Comment }
...
func (cl *CronLogic) DeleteCron(in *common_pb.IdReq) (*common_pb.SuccessResp, error) {
	if in.Id == 0 { return nil, status.Errorf(codes.InvalidArgument, "invalid cron id") }
