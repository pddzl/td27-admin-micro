...
func (l *ServiceTokenLogic) mapToResp(token *sysToolModel.ServiceToken) *service_token_pb.ServiceTokenResp {
	if token == nil {
		return nil
	}
...
func (l *ServiceTokenLogic) CreateServiceToken(in *service_token_pb.CreateServiceTokenReq) (*service_token_pb.CreateServiceTokenResp, error) {
	if in.Name == "" {
		return nil, status.Errorf(codes.InvalidArgument, "token name is required")
	}
	token := &sysToolModel.ServiceToken{Name: in.Name, Status: true}
	if in.Status != nil {
		token.Status = *in.Status
	}
	var ttl *time.Duration
	if in.ExpiresAt != nil {
		d := time.Duration(*in.ExpiresAt) * time.Second
		ttl = &d
	}
...
func (l *ServiceTokenLogic) GetServiceToken(in *common_pb.IdReq) (*service_token_pb.ServiceTokenResp, error) {
	if in.Id == 0 {
		return nil, status.Errorf(codes.InvalidArgument, "invalid token id")
	}
...
func (l *ServiceTokenLogic) AssignTokenPermissions(in *service_token_pb.AssignTokenPermissionsReq) (*common_pb.SuccessResp, error) {
	...
	for i, id := range in.PermissionIds {
		permIDs[i] = uint(id)
	}
...
func (l *ServiceTokenLogic) ValidateToken(in *service_token_pb.ValidateTokenReq) (*service_token_pb.ValidateTokenResp, error) {
	...
	if token != nil && token.Status {
		valid := true
		if token.ExpiresAt != nil && time.Now().Unix() >= *token.ExpiresAt {
			valid = false
		}
		if valid {
			resp.Valid = true
			resp.Token = l.mapToResp(token)
		}
	}
	return resp, nil
}
