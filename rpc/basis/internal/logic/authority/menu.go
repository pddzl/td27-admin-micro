...
func (ml *MenuLogic) mapMenuToResp(menu *authority.MenuModel) *menu_pb.MenuResp {
	if menu == nil {
		return nil
	}
...
func (ml *MenuLogic) mapMenuToTree(menu *authority.MenuModel, allMenus []*authority.MenuModel) *menu_pb.MenuTreeResp {
...
func (ml *MenuLogic) GetUserMenus(in *menu_pb.GetUserMenusReq) (*menu_pb.GetMenuTreeResp, error) {
	...
	tree := make([]*menu_pb.MenuTreeResp, 0)
	menuMap := make(map[uint]*authority.MenuModel)
