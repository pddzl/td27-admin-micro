...
func (ms *MenuServer) GetMenu(ctx context.Context, in *common_pb.IdReq) (*menu_pb.MenuResp, error) {
	ml := sysManagement.NewMenuLogic(ctx, ms.svcCtx)
	return ml.GetMenu(in)
}

func (ms *MenuServer) ListMenu(ctx context.Context, in *common_pb.PageReq) (*menu_pb.ListMenuResp, error) {
	ml := sysManagement.NewMenuLogic(ctx, ms.svcCtx)
	return ml.ListMenu(in)
}
